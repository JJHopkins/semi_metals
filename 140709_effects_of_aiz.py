...
#eiz_x = np.loadtxt('data/eiz_x_33.txt') # LDS in perpendicular direction
#eiz_z = np.loadtxt('data/eiz_z_33.txt') # LDS in perpendicular direction
#eiz_x = np.loadtxt('data/eiz_z_91.txt') # LDS of water, intervening medium
#eiz_w = np.loadtxt('data/eiz_z_93.txt') # LDS of water, intervening medium
...
#eiz_x = np.ones(len(eiz_z))#Aiz(eiz_x,eiz_z,eiz_w)
#eiz_w = np.ones(len(eiz_z))#Aiz(eiz_x,eiz_z,eiz_w)
...
#Ls = np.arange(1e-9,100e-9,20e-9)  # separation distance between 2 cyclinders
Ls = np.arange(1.e-9,100e-9,20e-9)  # separation distance between 2 cyclinders
...
for i,L in enumerate(Ls):
    ...
    for j,n in enumerate(ns):
        a[j] =  Aiz(eiz_x[j],eiz_z[j],eiz_w[j])
        delta[j] = Delta(eiz_z[j],eiz_w[j])
       ...
       # f2 = T*np.exp(-2.*np.sqrt(T*T+p[i,j]*p[i,j]))\
       #         /(T*T+p[i,j]*p[i,j])\
       #         *((T*T +2.*p[i,j]*p[i,j])*(T*T +2.*p[i,j]*p[i,j]))
...
#
        ...
        A2[i,j] = delta[j]*delta[j]*p[i,j]*p[i,j]*p[i,j]*p[i,j]*Ft2*(1.-a[j])*(1.-a[j])
