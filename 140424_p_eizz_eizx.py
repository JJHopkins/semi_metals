...
eiz_x = np.loadtxt('data/eiz_x_65.txt') # LDS in parallel direction
...
p_x = np.zeros(shape = (len(Ls),len(ns)))
p_z = np.zeros(shape = (len(Ls),len(ns)))
...
for i,L in enumerate(Ls):
    ...
    for j,n in enumerate(ns):
        ...
        p_x[i,j] = eiz_x[j]*Pn(eiz_w[j],zs[j],L)
        p_z[i,j] = eiz_z[j]*Pn(eiz_w[j],zs[j],L)
        p[p>1.10]= np.nan #NOTE: remove me later
        p_x[p_x>1.10*eiz_x[j]]= np.nan #NOTE: remove me later
        p_z[p_z>1.10*eiz_z[j]]= np.nan #NOTE: remove me later
fig = pl.figure()
ax = fig.gca(projection = '3d')
p_0 = pl.contour(LS,NS,p, rstride=1,cstride=1,alpha=0.5,cmap=cm.winter,  linewidth = 0.05, antialiased = True, shade = False)
p_1 = pl.contour(LS,NS,p_x, rstride=1,cstride=1,alpha=0.5,cmap=cm.autumn,  linewidth = 0.05, antialiased = True, shade = False)
p_2 = pl.contour(LS,NS,p_z, rstride=1,cstride=1,alpha=0.5,cmap=cm.spring,  linewidth = 0.05, antialiased = True, shade = False)
...
#np.savetxt('data/A0_65_p.txt',A0)
#np.savetxt('data/A2_65_p.txt',A2)
#np.savetxt('data/Ls_65_p.txt',Ls)
